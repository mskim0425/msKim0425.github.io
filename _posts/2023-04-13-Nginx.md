---
layout: post
title: "Nginx란? 역할과 핵심 설정 — 아파치와의 차이, 이벤트 기반 구조, 리버스 프록시"
description: "Nginx가 무엇이고 왜 쓰는지 — 아파치와 다른 이벤트 기반 비동기 구조, 리버스 프록시·로드밸런싱·정적 파일 서빙 역할과 기본 설정을 정리한다."
